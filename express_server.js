...
app.post("/login", (req, res) => {
  //checking if the user is in the database first before logging them in.
...
app.post("/register", (req, res) => {
  ...
  for (let id in usersDatabase) {
    if (email === usersDatabase[id]["email"]) {
      res.send("Email is the same try another email, thank you");
    }
  }
  if (email === "" || password === "") {
    res.send("400 bad request");
...
app.get("/urls", (req, res) => {
  if (!helper.getTemplateUserObjId(req, usersDatabase)) {
    ...
    const templateUrl = {
      userID: helper.getTemplateUserObjId(req, usersDatabase),
      urls: helper.userUrlDatabase(req, urlDatabase, usersDatabase)
    };
